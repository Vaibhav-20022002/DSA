...
// #1 Find the minimum of the elements in the range [l, r], where 0 <= l <= r <= n - 1.
...
public:
	void build(int ind, int low, int high, int arr[])
	{
		...
		// INTIALIZE THE NODES AS MINIMUM OF ITS CHILD NODES :
...
public:
	...
	{
		// IF THE GIVEN RANGE HAS NO OVERLAP WITH THE NODE RANGE, SIMPLY RETURN MAXIMUM VALUE TO AVOID ITS CONSIDERATION:
		...
		// LEFT AND RIGHT OF THE THAT PARTICULAR NODE AND RETURN THEIR MINIMUM :
		int mid = low + (high - low) / 2;
...
public:
	void update(int ind, int low, int high, int i, int val)
	{
		...
		// UPDATE EVERY NODE TO THE MINIMUM OF THEIR CHILDREN NODES :
		segment[ind] = min(segment[2 * ind + 1], segment[2 * ind + 2]);
	}
};
